...
public class Utils {
    ...
    public static Block putSignOn(Block block, BlockFace blockface, String line1, String line2, Material material) {
        ...
        if (newsign.getType() == Material.DARK_OAK_WALL_SIGN || newsign.getType() == Material.CRIMSON_WALL_SIGN) {
...
//		player.spigot().playEffect(block.getLocation().add(0.5, 0.5, 0.5), Effect.FLAME, 0, 0, 0.3F, 0.3F, 0.3F, 0.01F, 64, 64);
    ...
    public static void setCache(Block block, boolean access) {
        block.removeMetadata("expires", LockettePro.getInstance());
        block.removeMetadata("locked", LockettePro.getInstance());
        block.setMetadata("expires", new FixedMetadataValue(LockettePro.getInstance(), System.currentTimeMillis() + Config.getCacheTimeMillis()));
        block.setMetadata("locked", new FixedMetadataValue(LockettePro.getInstance(), access));
    }
    ...
    public static void resetCache(Block block) {
        ...
        for (BlockFace blockface : LocketteProAPI.newsfaces) {
            ...
            if (relative.getType() == block.getType()) {
                relative.removeMetadata("expires", LockettePro.getInstance());
                relative.removeMetadata("locked", LockettePro.getInstance());
            }
        }
    }
    ...
    public static void updateUuidByUsername(final Block block, final int line) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(LockettePro.getInstance(), () -> {
            ...
            if (uuid != null) {
                ...
                Bukkit.getScheduler().runTask(LockettePro.getInstance(), () -> setSignLine(block, line, towrite));
            }
        });
    }
