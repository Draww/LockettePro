...
import xyz.drawwdev.lockettepro.*;
...
public class BlockPlayerListener implements Listener {
    ...
    public void onPlayerQuickLockChest(PlayerInteractEvent event) {
        ...
        if (action == Action.RIGHT_CLICK_BLOCK && Tag.SIGNS.isTagged(player.getInventory().getItemInMainHand().getType())) {
            ...
            if (blockface == BlockFace.NORTH || blockface == BlockFace.WEST || blockface == BlockFace.EAST || blockface == BlockFace.SOUTH) {
                ...
                if (worldGuardDepend == null) return;
                if (worldGuardDepend.isProtectedFrom(block, player)) return; // blockwise
                if (worldGuardDepend.isProtectedFrom(block.getRelative(event.getBlockFace()), player))
                    return; // signwise
                ...
                if (LocketteProAPI.isLockable(block)) {
                    ...
                    if (!locked && !LocketteProAPI.isUpDownLockedDoor(block)) {
                        ...
                        coreProtectDepend.logPlacement(player, newsign);
                        ...
                        final CoreProtectDepend coreProtectDepend = LockettePro.getInstance().getDependencies().get(CoreProtectDepend.class);
                        if (coreProtectDepend == null) return;
                        coreProtectDepend.logPlacement(player, block.getRelative(blockface));
    ...
    public void onManualLock(SignChangeEvent event) {
        ...
        if (LocketteProAPI.isLockString(topline) || LocketteProAPI.isAdditionalString(topline)) {
            ...
            if (LocketteProAPI.isLockable(block)) {
                final WorldGuardDepend worldGuardDepend = LockettePro.getInstance().getDependencies().get(WorldGuardDepend.class);
                if (worldGuardDepend == null) return;
                if (worldGuardDepend.isProtectedFrom(block, player)) { // External check here
                    event.setLine(0, Config.getLang("sign-error"));
                    Utils.sendMessages(player, Config.getLang("cannot-lock-manual"));
                    return;
                }
                    ...
                    if (action == Action.RIGHT_CLICK_BLOCK) {
                        if ((LocketteProAPI.isDoubleDoorBlock(block) || LocketteProAPI.isSingleDoorBlock(block)) && LocketteProAPI.isLocked(block)) {
                            ...
                            if (closetime > 0) {
                                ...
                                for (Block door : doors) {
                                    door.setMetadata("lockettepro.toggle", new FixedMetadataValue(LockettePro.getInstance(), true));
                                }
                                Bukkit.getScheduler().runTaskLater(LockettePro.getInstance(), new DoorToggleTask(doors), closetime * 20L);
                            }
                        }
                    }
                }
    ...
    public void onBucketEmpty(PlayerBucketEmptyEvent event) {
        ...
        if (LocketteProAPI.isProtected(block) && !(LocketteProAPI.isOwner(block, player) || LocketteProAPI.isOwnerOfSign(block, player))) {
            ...
            }.runTaskLater(LockettePro.getInstance(), 1L);
        }
    }
