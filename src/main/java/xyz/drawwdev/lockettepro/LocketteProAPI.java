...
public class LocketteProAPI {
    ...
    public static boolean isUserOnSign(Block block, Player player) { // Requires (isLockSign or isAdditionalSign)
        ...
        for (int i = 1; i < 4; i++) {
            if (LockettePro.getInstance().getDependencies().get(VaultDepend.class).isPermissionGroupOf(lines[i], player))
                return true;
            if (LocketteProAPI.isScoreboardTeamOf(lines[i], player)) return true;
        }
        return false;
    }
